...
class GranularCredentialPermissions(BasePermission):
    """
    Permissão granular para credenciais.

    Hierarquia de verificação:
    1. Superuser/Staff: acesso total
    2. UserCredentialAccess: permissões granulares específicas
    3. Grupos Django: permissões genéricas
    """
    ...
    def has_permission(self, request, view):
        ...
        if request.method in SAFE_METHODS:
            # Permissão Django tradicional
            if request.user.has_perm('credentials.view_credential'):
                return True
            # Acesso granular: credenciais específicas via UserCredentialAccess
            from credentials.models import UserCredentialAccess
            return UserCredentialAccess.objects.filter(
                user=request.user, can_view=True
            ).exists()

        perms_map = {
            'POST': 'credentials.add_credential',
            'PUT': 'credentials.change_credential',
            'PATCH': 'credentials.change_credential',
    ...
    def has_object_permission(self, request, view, obj):
        ...
        from credentials.models import UserCredentialAccess
        try:
            access = UserCredentialAccess.objects.get(user=user, credential=obj)
            ...
            if request.method == 'DELETE':
                return user.has_perm('credentials.delete_credential')

        except UserCredentialAccess.DoesNotExist:
            if request.method in SAFE_METHODS:
                return user.has_perm('credentials.view_credential')

            if request.method in ['PUT', 'PATCH']:
                return user.has_perm('credentials.change_credential')
